...
- **キーの二重取り** — `create` と `keybindings.issues` が同じキーを持つ、または
  ダッシュボード自身のキー（`j` / `/` / `q` / `r` / `e` など）を奪う。押した側は
...
| `e` | 検索ボックスでアクティブなセクションの JQL を編集（セッション限り） |
| `r` | このセクションを再取得 |
| `y` / `Y` | 課題キー / URL をコピー |
| `?` | ヘルプ |
| `q` | 終了 |
...
タブバーと列ヘッダの間に、常時表示の全幅・枠付き検索ボックスがある。アクティブな
セクションの**実効 JQL**（override が今設定されていればそれ、なければセクション
自身の設定 `jql`）を表示し、`sprintPrefix` がある場合は `·` の後に付け足す。

`e` で編集にフォーカスする。`enter` で編集後の JQL でこのセクションを再取得、
`esc` で取消（何も変えない）。空（または空白のみ）で確定すると override が消え、
セクションの設定 JQL に戻って再取得する。

override は**セッション限りかつセクション単位**。`config.yml` には一切書き込まれ
ないので、ダッシュボードを再起動すれば全セクションが設定の JQL に戻る。override が
設定されている間は、手動の `r`、自動再取得の tick、フェッチ後の本文先読みの
どれも設定の JQL ではなく override を使う — 枠が編集後の JQL を表示している間、
設定ファイルのクエリに戻ってしまうことはない。

override が効いている間はボックスのアイコンに `*` が付く（`🔍` ではなく `🔍*`）
— 今表の行が `config.yml` の内容と食い違っていることが一目でわかるようにするため。

無効な JQL は Jira 自身のエラーとしてフッターに出る。表の行は消さず、前の内容が
残る。

編集中にセクションを切り替える（`tab` / `shift+tab` / `h` / `l` / 矢印）と、まず
編集は取消される — override はセクション単位で、切り替わった先でどちらに確定
させるつもりだったのか決めようがないため。

**破壊的変更**: `e` はダッシュボード自身が使う予約キーになった。
`keybindings.issues` や `create` で `e` を割り当てている設定は起動時に
「already a dashboard key」で落ちる — アップグレード前にそのキーを別のキーに
変更すること。
...
1つのキーが開けるものは1つで、検証が無ければ起きる「先に見たほうが黙って勝つ」
という曖昧な挙動を起動時に防ぐ。
...
`parent: true` を付けると、このキーは**カーソル下の行のサブタスク**を作る（同列の
課題ではなく）。

```yaml
create:
  - key: s
    type: サブタスク # このプロジェクトのサブタスク型であること
    parent: true
```

`type` はそのプロジェクトのサブタスク型である必要がある（設定側は Jira に照合できないので、
食い違いは起動時ではなく送信時に Jira 自身の拒否として現れる）。`parent: true` の作成では
スプリントを**設定しない** — サブタスクは親のスプリントを継承するもので、明示的に設定すると
多くの Jira 設定で拒否される。project は他の作成キーと同じく行から取る。
...
ライセンスは付与しない。このリポジトリは参照用として公開しているだけで、
GitHub の利用規約が認める閲覧とフォーク以外の権利は一切許諾しない。

コントリビュートは受け付けない。Issue は無効化してあり、Pull Request は
レビューせずに閉じる。
